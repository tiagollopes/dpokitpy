...
def generate_valid_rj_rg() -> str:
    """
    Gera RG RJ plausível:
    - 7 a 9 dígitos
...
def generate_invalid_rj_rg() -> str:
    invalid_options = [
        "1111111",
        "0000000",
        "99999999",
...
def build_sample():
    ...
    if choice == "RJ_INVALID":
        rg = generate_invalid_rj_rg()
        text = random.choice([
            f"RG RJ {rg}",
            f"Identidade {rg}",
            f"Documento de identidade {rg}",
...
def run_benchmark(iterations=2000):
    ...
    log_path = os.path.join(os.path.dirname(__file__), "erros_rg_rj.txt")
    with open(log_path, "w", encoding="utf-8") as f:
        if errors:
            f.write("RELATÓRIO DE FALHAS - RG RJ\n")
